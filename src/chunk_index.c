...
chunk_index_mapping_from_tuple(TupleInfo *ti, ChunkIndexMapping * cim)
{
	FormData_chunk_index *chunk_index = (FormData_chunk_index *) GETSTRUCT(ti->tuple);
	Chunk	   *chunk = chunk_get_by_id(chunk_index->chunk_id, 0, true);
	Hypertable  *hyper = chunk_get_hypertable(chunk);
	Oid			nspoid_chunk = get_rel_namespace(chunk->table_id);
	Oid			nspoid_hyper = get_rel_namespace(hyper->main_table_relid);
	if (cim == NULL) {
	...
	cim->hypertableoid = hyper->main_table_relid;
...
chunk_index_recreate_create(PG_FUNCTION_ARGS)
{
	...
	hypertable_rel = relation_open(cim->hypertableoid, AccessShareLock);
	hypertable_index_rel = relation_open(cim->parent_indexoid, AccessShareLock);

	/* Need ShareLock on the heap relation we are creating indexes on */
	chunk_rel = relation_open(chunk_index_rel->rd_index->indrelid, ShareLock);

	constraint_oid = get_index_constraint(cim->parent_indexoid);

	new_chunk_indexrelid = chunk_relation_index_create(hypertable_rel, hypertable_index_rel, 
													   chunk_rel, OidIsValid(constraint_oid));

	relation_close(chunk_rel, NoLock);

	relation_close(hypertable_index_rel, AccessShareLock);
	relation_close(hypertable_rel, AccessShareLock);
